...
func TestTaskQueueDefault(t *testing.T) {
	if os.Getenv("TEMPORAL_TASK_QUEUE") == "" && TaskQueue() != "lesson-l2_error_handling-tq" {
		t.Errorf("TaskQueue() default = %q, want lesson-l2_error_handling-tq", TaskQueue())
	}
}

// TestOnboardingWorkflow — юнит-тест на TestWorkflowEnvironment: активности
...
func TestOnboardingWorkflow(t *testing.T) {
	...
	env.OnActivity(CheckKYC, mock.Anything).Return("ok", nil)
	env.OnActivity(IssueCard, mock.Anything).Return("ok", nil)
	env.OnActivity(Cleanup, mock.Anything).Return("ok", nil)
	env.ExecuteWorkflow(OnboardingWorkflow)
...
func TestIntegration(t *testing.T) {
	...
	// стартани OnboardingWorkflow через client.ExecuteWorkflow и проверь результат
	// поведения урока «Обработка ошибок».
}
